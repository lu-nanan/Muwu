...
def login(request):
    # 1. 获取请求参数
    account = request.data.word_search('account')
    password = request.data.word_search('password')
    # 2. 参数验证
...
def verification(request):
    try:
        # 1. 获取并验证邮箱
        email = request.data.word_search('email')
        print(email)
        if not email:
...
def register(request):
    try:
        # 1. 获取注册数据
        email = request.data.word_search('email')
        verification_code = request.data.word_search('verification')
        password = request.data.word_search('password')
